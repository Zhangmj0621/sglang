...
def is_gemm_ar_eligible(*, m: int, n: int, k: int, world_size: int) -> bool:
    ...
    The tile shape is chosen PER CALL: ``op.config_for(m)`` picks
    ``(tile_m, tile_n, cluster_m)`` from the compiled table using this call's
    token count, on the same symmetric memory, so a 512-token chunk and a
    16384-token chunk each run the configuration tuned for their own m. This gate
    therefore asks "does any compiled tile shape fit this m?" rather than testing
    m against one fixed shape:

    * ``config_for(m) is not None`` covers the M alignment
      (``m % (tile_m * world_size)``, whose violation makes swizzle_m_tile
      non-bijective and the reduce warp's ``owner`` index run past a PEER rank's
      tile-flag table -- a wild REMOTE store), the
      ``m_tiles % cluster_m`` grid constraint, the row-stride bound on
      ``n / tile_n``, and the ``tile_m_cnt_per_rank`` capacity.
    * ``k % kTileK``: the K-tile, derived from the element size (see
      ``_kernel_tile_k``).
    * ``m <= op.max_m``: the instance's own buffers (tile_flags, sq, counters)
    ...
    if op.config_for(m) is None:
        # No compiled (tile_m, tile_n, cluster_m) fits this m. All three are
        # per-call runtime arguments of the op, so this is NOT "m is not a
        # multiple of some instance-fixed tile_m" -- it means no instantiated
        # tile shape divides this m evenly while also satisfying
        # m_tiles % cluster_m == 0 within the sized buffers.
        warn_fallback(
...
def try_forward(
    *,
    ...
    # tile_m_cnt_per_rank = m // tile_m // world_size for THIS call's m AND
    # THIS call's tile_m -- op.config_for(m) picks tile_m per call, so read it
    # from there rather than from the instance's construction-time value.
    # Compute that literal expression -- not a different one that merely happens
    # to coincide -- because the residual marker below tells every later
    # consumer which rows are fresh: if the alignment gate were ever weakened,
    # this is the check that must catch the divergence instead of silently
    # mis-marking rows.
