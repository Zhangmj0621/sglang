...
class ReqToTokenPool:
    ...
    def __init__(
        ...
        with memory_saver_adapter.region(GPU_MEMORY_TYPE_KV_CACHE):
            ...
            # Parallel table for non-resident layer positions (dynamic pages)
            self.dynamic_req_to_token = (
                torch.zeros(
                    (size, max_context_len), dtype=torch.int32, device=device
                )
                if enable_dynamic_kvcache_layout
                else None
            )
    ...
    def write_dynamic(self, indices, values):
        if self.dynamic_req_to_token is not None:
            self.dynamic_req_to_token[indices] = values
...
class DynamicMHATokenToKVPool(KVCache):
    ...
    def __init__(
        ...
        # Per-layer stride: used to strip slot_start encoding from flat indices
        self.pool_stride = self.size + self.page_size
    ...
    def _create_buffers(self):
        with self.memory_saver_adapter.region(GPU_MEMORY_TYPE_KV_CACHE):
            with (
                ...
                # Allocate as contiguous 4D tensors: [layer_num, size+page_size, head_num, head_dim]
                # The padded slot 0 is used for writing dummy outputs from padded tokens.
                self._k_buffer_all = torch.zeros(
                    (
                        self.layer_num,
                ...
                self._v_buffer_all = torch.zeros(
                    (
                        self.layer_num,
                        self.size + self.page_size,
                        self.head_num,
        ...
        # Per-layer views for backward compatibility (each is C-contiguous)
        self.k_buffer = [self._k_buffer_all[i] for i in range(self.layer_num)]
        self.v_buffer = [self._v_buffer_all[i] for i in range(self.layer_num)]

        # Flat views: [layer_num * (size+page_size), head_num, head_dim]
        # Used by the dynamic KV cache layout so that a single encoded index
        # (embedding both layer and position) directly addresses the buffer.
        self.k_buffer_flat = self._k_buffer_all.view(-1, self.head_num, self.head_dim)
        self.v_buffer_flat = self._v_buffer_all.view(
    ...
    def _clear_buffers(self):
        del self.k_buffer
        del self.v_buffer
        del self.k_buffer_flat
        del self.v_buffer_flat
        del self._k_buffer_all
        del self._v_buffer_all
    ...
    def _get_key_buffer(self, layer_id: int):
        # Per-layer view for internal use (host transfer, halving, etc.)
        if self.store_dtype != self.dtype:
            return self.k_buffer[layer_id - self.start_layer].view(self.dtype)
        return self.k_buffer[layer_id - self.start_layer]

    def _get_flat_key_buffer(self):
        if self.store_dtype != self.dtype:
            return self.k_buffer_flat.view(self.dtype)
        return self.k_buffer_flat

    def get_key_buffer(self, layer_id: int):
        if self.layer_transfer_counter is not None:
            self.layer_transfer_counter.wait_until(layer_id - self.start_layer)
        return self._get_flat_key_buffer()
    ...
    def _get_flat_value_buffer(self):
        if self.store_dtype != self.dtype:
            return self.v_buffer_flat.view(self.dtype)
        return self.v_buffer_flat

    def get_value_buffer(self, layer_id: int):
        if self.layer_transfer_counter is not None:
            self.layer_transfer_counter.wait_until(layer_id - self.start_layer)
        return self._get_flat_value_buffer()
