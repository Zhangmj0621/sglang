...
SGLANG_PYTHON_ROOT = str(
    (__import__("pathlib").Path(__file__).resolve().parents[3] / "python")
)
...
for _pkg in _MOCK_PACKAGES:
    if _pkg not in sys.modules:
        ...
        _m.__package__ = _pkg
        _m.__name__ = _pkg
        _m.__spec__ = types.SimpleNamespace(name=_pkg, submodule_search_locations=[])
        sys.modules[_pkg] = _m
...
#     Tolerant: skip if the stub helper does not exist in this checkout.
# ---------------------------------------------------------------------------
_triton_stub_path = f"{SGLANG_PYTHON_ROOT}/sglang/_triton_stub.py"
if "triton" not in sys.modules and os.path.exists(_triton_stub_path):
...
#     distributed/parallel_state.py). Tolerant: skip if absent.
# ---------------------------------------------------------------------------
_mps_stub_path = f"{SGLANG_PYTHON_ROOT}/sglang/_mps_stub.py"
if os.path.exists(_mps_stub_path):
