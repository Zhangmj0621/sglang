...
async def run_benchmark(args):
    ...
    try:
        # /release_ref answers an untracked rid with success=True and
        # "rid <rid> not tracked" when the feature is on; with success=False and
        # "ref-aware KV buffer not enabled" when it is off; and send_release_ref
        # synthesizes success=False for a non-200 (e.g. HTTP 404 on old servers).
        # So `success` is the authoritative signal; the message match is only a
        # fallback for older response wordings.
        resp_data = await send_release_ref(base_url, "__probe__")
        msg = str(resp_data.get("message", ""))
        enable_ref_api = bool(resp_data.get("success", False)) or (
            "not tracked" in msg or "not found" in msg or "released" in msg
        )
